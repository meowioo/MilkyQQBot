...
public class CommandContext
{
    ...
    public string[] Args { get; set; } // 指令参数
    ...
    public async Task ReplyImageAsync(string imageUrl)
    {
        try
        {
            var segment = new OutgoingSegment<ImageOutgoingSegmentData>(
    ...
    public async Task ReplyVideoAsync(string videoUrl, string? thumbUrl = null)
    {
        try
        {
            var segment = new OutgoingSegment<VideoOutgoingSegmentData>(
            ...
            await SendSegmentsAsync(segment);
            Console.WriteLine($"[视频已发送] -> {Scene}:{PeerId}");
        }
        catch (Exception ex)
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine($"[视频发送失败] {ex.Message}");
            Console.ResetColor();

            // 视频段失败时，至少把视频链接发出去
            await ReplyAsync($"[视频链接]\n{videoUrl}");
        }
    }
