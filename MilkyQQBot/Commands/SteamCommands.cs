...
public static class SteamCommands
{
    public static void Register(CommandHandler commandHandler, MilkyClient milky)
    {
        commandHandler.RegisterCommand("/绑定steam", async (context) =>
        {
            ...
            if (parts.Length < 2)
            {
                await context.TextAsync("格式错误！请使用：/绑定steam 你的SteamID\n例如：/绑定steam 76561198123456789");
                return;
            }
            ...
            {
                await context.TextAsync($"🔍 检测到输入为自定义链接 [{inputId}]，正在向 G胖 申请解析真实 ID...");
                finalSteamId = await SteamSpyService.ResolveVanityUrlAsync(inputId);

                if (string.IsNullOrEmpty(finalSteamId))
                {
                    await context.TextAsync("❌ 绑定失败：无法解析该自定义链接。请检查是否输入正确，或者直接输入 17 位的纯数字 SteamID。");
                    return;
                }
            }
            ...
            {
                await context.TextAsync($"✅ 绑定成功！你的 QQ 已经与 SteamID [{finalSteamId}] 灵魂互绑！\n以后大家可以通过【/steam查岗】来视奸你的游玩状态了！");
            }
            else
            {
                await context.TextAsync($"✅ 解析并绑定成功！\n短链接 [{inputId}] 对应的真实 ID 为：{finalSteamId}\n以后大家可以通过【/steam查岗】来视奸你的游玩状态了！");
            }
        });
        ...
        commandHandler.RegisterCommand("/steam查岗", async (context) =>
        {
            ...
            if (string.IsNullOrEmpty(steamId))
            {
                ...
                await context.TextAsync($"❌ 查岗失败：{tips}\n请发送【/绑定steam 你的17位ID，17位ID可通过https://steamid.io/输入用户名进行查询】进行绑定。");
                return;
            }

            await context.TextAsync("📡 正在潜入 G胖 的服务器，拉取数据，请稍候...");
            ...
            try
            {
                ...
                if (spyResult == null)
                {
                    await context.TextAsync("❌ 获取情报失败，可能网络开小差了。");
                    return;
                }
            ...
            {
                await context.TextAsync($"❌ 调取数据时发生错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/steam游戏库", async (context) =>
        {
            ...
            {
                string tips = targetQqId == context.SenderId ? "你还没绑定呢！" : "TA还没绑定呢！";
                await context.TextAsync($"❌ 查询失败：{tips}\n请发送【/绑定steam 你的17位ID，17位ID可通过https://steamid.io/输入用户名进行查询】进行绑定。");
                return;
            }

            await context.TextAsync("🎮 正在盘点该玩家的赛博资产底裤，请稍候...");
            ...
            try
            {
                ...
                if (libResult == null)
                {
                    await context.TextAsync("❌ 获取游戏库失败，网络可能开小差了。");
                    return;
                }
            ...
            {
                await context.TextAsync($"❌ 盘点资产时发生错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/steam等级", async (context) =>
        {
            ...
            await context.TextAsync("💳 正在查询 G胖 的韭菜登记册，请稍候...");
            ...
            try
            {
                ...
                if (levelResult == null)
                {
                    await context.TextAsync("❌ 获取等级信息失败，网络可能开小差了。");
                    return;
                }
        ...
        commandHandler.RegisterCommand("/csgo库存", async (context) =>
        {
            ...
            await context.TextAsync("📦 正在撬开该玩家的仓库卷帘门，搜刮值钱的饰品，请稍候...");
            ...
            try
            {
                ...
                if (invResult == null)
                {
                    await context.TextAsync("❌ 访问库存失败，可能是接口限流了，请过会儿再试。");
                    return;
                }
            ...
            {
                await context.TextAsync($"❌ 扒底裤时发生错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/csgo开箱", async (context) =>
        {
            ...
            await context.TextAsync("🎰 正在为您开启【穷鬼专属武器箱】... 正在滚动指针...");
            ...
            try
            {
                ...
                if (unboxResult.IsRare)
                {
                    ...
                    await context.TextAsync(rareTips);
                }
            ...
            {
                await context.TextAsync($"❌ 武器箱卡壳了：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/l4d2", async (context) =>
        {
            ...
            await context.TextAsync("💉 正在入侵 C.E.D.A. 官方数据库，调取目标体检档案...");
            ...
            try
            {
                ...
                if (l4dResult == null)
                {
                    await context.TextAsync("❌ 档案读取失败，可能是网络抽风了。");
                    return;
                }
            ...
            {
                await context.TextAsync($"❌ 调取档案时发生异常：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/steam热销", async (context) =>
        {
            ...
            await context.TextAsync("🔥 正在抓取 Steam 实时热销榜单，请稍候...");
            ...
            try
            {
                ...
                if (games.Count == 0)
                {
                    await context.TextAsync("❌ 糟糕，访问 Steam 接口失败，G胖可能把网线拔了...");
                    return;
                }
            ...
            {
                await context.TextAsync($"❌ 生成榜单时发生严重错误：{ex.Message}");
            }
        });
    }
