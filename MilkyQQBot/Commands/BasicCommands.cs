    ...
    public static void Register(CommandHandler commandHandler, MilkyClient milky)
    {
        commandHandler.RegisterCommand("/help", async (context) =>
        {
            ...
            try
            {
                ...
                await context.ReplyImageAsync(base64ImageUrl);
            }
            catch (Exception ex)
            {
                await context.ReplyAsync($"❌ 生成说明书失败：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/epic", async (context) =>
        {
            ...
            await context.ReplyAsync("⏳ 正在为您生成Epic当前限时免费、即将免费的游戏详情海报，请稍候...");
            try
            {
                await context.ReplyImageAsync(ApiConfig.ApiUrls.EpicFreeGames);
            }
            catch (Exception ex)
            {
                await context.ReplyAsync($"❌ 获取 Epic 数据失败，发生了错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/摸鱼日报", async (context) =>
        {
            ...
            await context.ReplyAsync("🐟 正在为您获取今日份的摸鱼日历，请稍候...");
            try
            {
                await context.ReplyImageAsync(ApiConfig.ApiUrls.MoyuCalendar);
            }
            catch (Exception ex)
            {
                await context.ReplyAsync($"❌ 获取摸鱼日报失败，发生了错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/今日群活跃", async (context) =>
        {
            ...
            if (topUsers.Count == 0)
            {
                await context.ReplyAsync("今天群里真冷淡，还没有人讲话");
                return;
            }
            ...
            await context.ReplyAsync("📊 正在统计今日群活跃数据并绘制榜单，请稍候...");
            try
            {
            ...
            {
                await context.ReplyAsync($"❌ 生成排行榜失败，发生了错误：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/本周群活跃", async (context) =>
        {
            ...
            if (topUsers.Count == 0)
            {
                await context.ReplyAsync("这周群里居然一条消息都没有，大家都太高冷了吧！");
                return;
            }
            ...
            await context.ReplyAsync("📊 正在统计近 7 天的数据并生成【本周群活跃榜】，请稍候...");
            try
            {
            ...
            {
                await context.ReplyAsync($"❌ 生成周榜失败：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/今日词云", async (context) =>
        {
            ...
            if (messages.Count == 0)
            {
                await context.ReplyAsync("今天群里太安静了，连一句话都没有，生成不了词云呢~");
                return;
            }

            await context.ReplyAsync("☁️ 正在进行语义分析并渲染今日词云，请稍候...");
            try
            {
                ...
                {
                    await context.ReplyAsync("群里大家发的话太短或者全是被过滤掉的水词，提取不到有效关键词哦！");
                }
            }
            catch (Exception ex)
            {
                await context.ReplyAsync($"❌ 生成词云失败：{ex.Message}");
            }
        });
        ...
        commandHandler.RegisterCommand("/群防撤回", async (context) =>
        {
            ...
            if (isNowEnabled)
            {
                await context.ReplyAsync("✅ 已开启群防撤回。群员撤回消息将被机器人公开处刑！");
            }
            else
            {
                await context.ReplyAsync("❌ 已关闭群防撤回。");
            }
        });
        ...
        commandHandler.RegisterCommand("/群ai", async (context) =>
        {
            ...
            if (isNowEnabled)
            {
                await context.ReplyAsync("✅ 已开启本群 AI 阴阳怪气功能。");
            }
            else
            {
        ...
        commandHandler.RegisterCommand("/find", async (context) =>
        {
            ...
            {
                await context.ReplyAsync("⚠️ 缺少查询目标！\n格式：/find [QQ/手机号/@微博uid]");
                return;
            }

            await context.ReplyAsync($"⏳ 奈奈川正在潜入深层数据网，检索 [{target}] 的情报，请稍候...");
            ...
            // 5. 输出结果
            await context.ReplyAsync(queryResult);
        });
        ...
        commandHandler.RegisterCommand("/接码项目", async (context) =>
        {
            ...
            {
                await context.ReplyAsync("⚠️ 格式错误！请输入：/接码项目 [项目关键词]\n例如：/接码项目 抖音");
                return;
            }

            await context.ReplyAsync($"⏳ 正在云端检索包含[{keyword}]的项目，请稍候...");
        ...
        commandHandler.RegisterCommand("/取号", async (context) =>
        {
            ...
            {
                await context.ReplyAsync("⚠️ 格式错误！请输入：/取号 [项目ID]\n例如：/取号 1001");
                return;
            }

            await context.ReplyAsync($"📡 正在向平台申请项目 [{projectId}] 的手机号...");
        ...
        commandHandler.RegisterCommand("/拿码", async (context) =>
        {
            ...
            if (parts.Length != 3)
            {
                await context.ReplyAsync("⚠️ 格式错误！请输入：/拿码 [项目ID] [手机号]\n例如：/拿码 1001 17012345678");
                return;
            }
            ...
            await context.ReplyAsync("📩 正在拦截短信中心，尝试读取验证码...");
